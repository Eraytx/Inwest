...
def init_db():
    ...
    # 3. Transactions table (Added user_id)
    cursor.execute(f"""
    ...
    # 4. Portfolio history table (Added user_id)
    cursor.execute("""
...
# --- Data Functions (Modified to include user_id) ---
...
def add_transaction(user_id, symbol, asset_class, tx_type, amount, price, timestamp=None):
    ...
    try:
        cursor.execute(
            f"INSERT INTO transactions (user_id, symbol, type, amount, price, timestamp) VALUES ({ph}, {ph}, {ph}, {ph}, {ph}, {ph})",
            (user_id, symbol, tx_type, amount, price, timestamp)
        )
        ...
        if tx_type == "BUY":
            if asset:
                ...
                cursor.execute(f"UPDATE assets SET amount = {ph}, avg_price = {ph} WHERE user_id = {ph} AND symbol = {ph}", (new_amount, new_avg, user_id, symbol))
            else:
                cursor.execute(f"INSERT INTO assets (user_id, symbol, asset_class, amount, avg_price) VALUES ({ph}, {ph}, {ph}, {ph}, {ph})", (user_id, symbol, asset_class, amount, price))
        elif tx_type == "SELL":
            if asset:
                ...
                if new_amount <= 1e-8:
                    ...
                    cursor.execute(f"UPDATE assets SET amount = {ph} WHERE user_id = {ph} AND symbol = {ph}", (new_amount, user_id, symbol))
            else:
                cursor.execute(f"INSERT INTO assets (user_id, symbol, asset_class, amount, avg_price) VALUES ({ph}, {ph}, {ph}, {ph}, {ph})", (user_id, symbol, asset_class, -amount, price))
        conn.commit()
        return True
...
def save_report(user_id, report_type, report_text, meta_data=None):
    ...
    ph = "%s" if DATABASE_URL else "?"
    cursor.execute(f"INSERT INTO reports (user_id, report_type, generated_at, report_text, meta_data) VALUES ({ph}, {ph}, {ph}, {ph}, {ph})", (user_id, report_type, generated_at, report_text, json.dumps(meta_data) if meta_data else None))
    conn.commit()
    conn.close()
...
def recalculate_assets(user_id):
    ...
    try:
        ...
        for tx in txs:
            ...
            amount = tx["amount"]
            price = tx["price"]
            if symbol.endswith(".IS"): asset_class = "BIST"
            elif "-" in symbol: asset_class = "Crypto"
            else: asset_class = "US_Stocks"
            cursor.execute(f"SELECT amount, avg_price FROM assets WHERE user_id = {ph} AND symbol = {ph}", (user_id, symbol))
            asset = cursor.fetchone()
            if tx_type == "BUY":
                if asset:
                ...
                if asset:
                    new_amount = max(0.0, asset["amount"] - amount)
                    if new_amount <= 1e-8: cursor.execute(f"DELETE FROM assets WHERE user_id = {ph} AND symbol = {ph}", (user_id, symbol))
                    else: cursor.execute(f"UPDATE assets SET amount = {ph} WHERE user_id = {ph} AND symbol = {ph}", (new_amount, user_id, symbol))
                else:
                    cursor.execute(f"INSERT INTO assets (user_id, symbol, asset_class, amount, avg_price) VALUES ({ph}, {ph}, {ph}, {ph}, {ph})", (user_id, symbol, asset_class, -amount, price))
        conn.commit()
    except Exception as e:
